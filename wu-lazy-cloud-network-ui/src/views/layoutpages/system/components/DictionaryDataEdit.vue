...
const { code, name, description, dictionaryCode, type } = toRefs(form);
...
rowData.value &&
    ...
    (dictionaryCode.value = rowData.value.dictionaryCode),
...
const onSubmit = () => {
    formRef.value.validate(async (valid) => {
        if (valid) {
            let res;
            if (title.value === "添加") {
                res = await VE_API.system.dictionaryDataAdd(form);
            } else {
                res = await VE_API.system.dictionaryDataEdit({
                    id: rowData.value.id,
                    ...form,
                });
            }
