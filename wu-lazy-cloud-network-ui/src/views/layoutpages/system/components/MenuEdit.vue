<template>
    ...
    >
        ...
        >
            ...
            <el-form-item label="按钮" prop="upsertMenu" v-show="type === 2">
    ...
    return files
        ...
        .map((key) => {
            return {
                ...
                buttons:
                    files(key).default.data &&
                    (files(key).default.data().buttons || []),
            };
        })
        .filter((key) => {
...
const { name, type, parentCode, menu, url, icon, iframe, sort, toPath, code } =
    toRefs(form);
/**
 * @description: 字段重置
 * @param {*}
...
rowData.value &&
    ...
    (parentCode.value = rowData.value.parentCode),
...
const menuOptions = computed(() => {
    ...
    let _item = XE.findTree(
        menuList.value,
        (item) => item.code === parentCode.value,
    );
    if (_item && files.find((item) => item.url === _item.item.url)) {
        return files.find((item) => item.url === _item.item.url).buttons;
    } else {
        return false;
    }
    // }
...
const watchTitle = () => {
    if (title.value === "添加按钮" || title.value === "添加子级") {
        resetForm();
        parentCode.value = treeFindPath(
            menuList.value,
            (data) => data.code === rowData.value.code,
            "code",
        ).slice(-1)[0];
